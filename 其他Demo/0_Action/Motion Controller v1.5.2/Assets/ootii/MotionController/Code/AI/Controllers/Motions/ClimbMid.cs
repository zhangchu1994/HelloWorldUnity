...
namespace com.ootii.AI.Controllers
{
    ...
    public class ClimbMid : MotionControllerMotion
    {
        ...
        public override void LoadAnimatorData()
        {
            ...
            mController.AddAnimatorName("Entry -> " + lBaseLayer + "ClimbMid-SM.IdleClimbMid");
            mController.AddAnimatorName("AnyState -> " + lBaseLayer + "ClimbMid-SM.IdleClimbMid");

            mController.AddAnimatorName(lBaseLayer + "ClimbMid-SM.IdleClimbMid");
            mController.AddAnimatorName(lBaseLayer + "ClimbMid-SM.IdleClimbMid -> " + lBaseLayer + "ClimbMid-SM.ClimbRecoverIdle");
            mController.AddAnimatorName(lBaseLayer + "ClimbMid-SM.ClimbRecoverIdle");
        }
        ...
        public override bool TestActivate()
        {
            ...
            if (!ootiiInputStub.IsJustPressed("Jump")) { return false; }
        ...
        public override void UpdateMotion()
        {
            ...
            // Orient ourselves towards the anchor point
            if (lStateName == lBaseLayer + "ClimbMid-SM.IdleClimbMid")
            {
            ...
            else if (lStateName == lBaseLayer + "ClimbMid-SM.ClimbRecoverIdle")
            {
                // As we exit the final animation, move towards the exact position that the
                // following animation (usually idle) will match to.
                Vector3 lTargetPosition = mController.transform.position + mExitPositionOffset;
        ...
        public override void OnAnimatorStateChange(int rLastStateID, int rNewStateID)
        {
            ...
            if (rNewStateID == mController.AnimatorStateIDs[lBaseLayer + "ClimbMid-SM.ClimbRecoverIdle"])
            {
                mPhase = ClimbMid.PHASE_TO_TOP;
            }

            // If we've moved out the recovery state, reset the motion and state
            if (rLastStateID == mController.AnimatorStateIDs[lBaseLayer + "ClimbMid-SM.ClimbRecoverIdle"])
            {
        ...
        protected bool IsInClimbUpState
        {
            get
            {
                ...
                if (lTransition == "Entry -> " + lBaseLayer + "ClimbMid-SM.IdleClimbMid" ||
                    lTransition == "AnyState -> " + lBaseLayer + "ClimbMid-SM.IdleClimbMid" ||
                    lState == lBaseLayer + "ClimbMid-SM.IdleClimbMid"
                    )
                {
                    return true;
                }
