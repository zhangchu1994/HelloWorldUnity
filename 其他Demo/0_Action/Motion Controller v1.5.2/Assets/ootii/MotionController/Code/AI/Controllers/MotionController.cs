...
namespace com.ootii.AI.Controllers
{
    ...
    public class MotionController : Controller
    {
        ...
        /// </summary>
        //protected float mColliderRadius = 0.3f;
        [Obsolete("Use ActorRadius to determine the actors true radius.")]
        public override float ColliderRadius
        {
            get { return mActorRadius; }
        }
        ...
        /// </summary>
        [Obsolete("Use ActorHeight to determine the actor's true height.")]
        public float ColliderHeight
        {
            get { return mActorHeight; }
        }
        ...
        /// </summary>
        [Obsolete("Use ActorScale to determine the actor's relative scale for motions.")]
        public float CharacterScale
        {
            get { return gameObject.transform.localScale.y * (mActorHeight / 1.8f); }
        }

        /// <summary>
        /// Default distance from the character to the ground
        /// </summary>
        protected float mActorHeight = 1.8f;
        ...
        /// Default distance from the character to the ground
        /// </summary>
        protected Vector3 mActorCenter = new Vector3(0f, 0.9f, 0f);
        public Vector3 ActorCenter
        {
            get { return mActorCenter; }
        }

        /// <summary>
        /// Default distance from the character to the ground
        /// </summary>
        protected float mActorToGroundDistance = 0.9f;
        public float ActorToGroundDistance
        {
            get { return mActorToGroundDistance; }
        }

        /// <summary>
        /// Radius of the character as it's standing
        /// </summary>
        protected float mActorRadius = 0.3f;
        ...
        /// <summary>
        /// Height at which we can move up onto obstacles
        /// without any extra effort. Used for stairs and such.
        /// </summary>
        protected float mMaxStepHeight = 0.3f;
        public float MaxStepHeight
        {
            get { return mMaxStepHeight; }
        }

        /// <summary>
        /// Max angle the actor can climb up
        /// </summary>
        protected float mMaxSlopeAngle = 45f;
        public float MaxSlopeAngle
        {
            get { return mMaxSlopeAngle; }
        }

        /// <summary>
        /// Amount to sink the character when he is on a slope. the
        /// value will go from 0 to max as the slope goes from 0 to 45 degrees.
        /// </summary>
        public float _MaxSlopeSink = 0.0f;
        public float MaxSlopeSink
        {
            get { return _MaxSlopeSink; }
            set { _MaxSlopeSink = value; }
        }

        /// <summary>
        /// Current sink amount
        /// </summary>
        public float mSlopeSink = 0f;
        public float SlopeSink
        {
            get { return mSlopeSink; }
        }

        /// <summary>
        /// Determines how quickly the actor steps up to meet the new height
        /// </summary>
        public float _ActorStepUpLerp = 0.3f;
        public float ActorStepUpLerp
        {
            get { return _ActorStepUpLerp; }
            set { _ActorStepUpLerp = value; }
        }

        /// <summary>
        /// Determines how quickly the actor steps down to meet the new height
        /// </summary>
        public float _ActorStepDownLerp = 0.5f;
        public float ActorStepDownLerp
        {
            get { return _ActorStepDownLerp; }
            set { _ActorStepDownLerp = value; }
        }
        ...
        /// </summary>
        [Obsolete("This field is obsolete. Use ActorHeight for the true actor's height.")]
        ...
        /// <summary>
        /// Target step height we're trying to reach
        /// </summary>
        protected float mActorYPositionTarget = 0f;

        /// <summary>
        /// Current height as we move towards the target
        /// </summary>
        protected float mActorYPositionAdjust = 0f;

        /// <summary>
        /// Keeps track of the original collider height so we can reset as needed
        /// </summary>
        ...
        /// </summary>
        protected Vector3 mBaseColliderCenter = new Vector3(0, 0.9f, 0);
        ...
        public void Start()
        {
            ...
            mActorHeight = (mCharController.height / 2f) + mCharController.center.y;
            mActorCenter = new Vector3(mCharController.center.x, mActorHeight / 2f, mCharController.center.z);
            mActorToGroundDistance = mActorCenter.y;
            mActorRadius = mCharController.radius;
            mMaxStepHeight = mCharController.stepOffset;
            mMaxSlopeAngle = mCharController.slopeLimit;

            //mColliderHeight = mCharController.height;
            //float lColliderGap = mCharController.center.y - (mColliderHeight / 2f);
            //mColliderCenter = new Vector3(mCharController.center.x, (mColliderHeight + lColliderGap) / 2f, mCharController.center.z);
            //mColliderRadius = mCharController.radius;

            // If a bumper is requested, and missing defaults, create them
            if (_ForwardBumper.sqrMagnitude > 0f)
            {
                if (mCharController != null && _ForwardBumper.y == 0f) { _ForwardBumper.y = mMaxStepHeight + 0.1f; }
                if (mCharController != null && _ForwardBumper.z == 0f) { _ForwardBumper.z = mActorRadius + 0.1f; }
            }
        ...
        public void Update()
        {
            ...
            if (MotionLayers.Count > 0 && MotionLayers[0].UseTrendData) { lUseTrendData = true; }
#if MC_ENABLE_PROFILING
           ...
           SetAnimatorProperties(mState, lUseTrendData);
            ...
            if (_UseInput)
            {
                Log.FileScreenWrite(String.Format("AC.Update() Time:{0:f1}ms {1} Mx:{2:f4} My:{3:f4} Vx:{4:f4} Vy:{5:f4} MCTime:{6:f4}ms", Time.deltaTime * 1000f, CurrentMotionName, ootiiInputStub.MovementX, ootiiInputStub.MovementY, ootiiInputStub.ViewX, ootiiInputStub.ViewY, mUpdateProfiler.Time), 2);
                //Log.FileScreenWrite(String.Format("AC.Update() IsGrd:{0} GrdDist:{1:f4} GrdA:{2:f4} IsFwdBlocked:{3} Pos:{4} Vel:{5}", mState.IsGrounded, mState.GroundDistance, mState.GroundAngle, mState.IsForwardPathBlocked, StringHelper.ToString(transform.position), StringHelper.ToString(mState.Velocity)), 3);
                //Log.FileScreenWrite(String.Format("AC.Update() Motion:{0} MotionDur:{1:f4} StPhase:{2:f4} Anim:{3} AT:{4}", CurrentMotionName, CurrentMotionDuration, mState.AnimatorStates[0].MotionPhase, AnimatorHashToString(mState.AnimatorStates[0].StateInfo, mState.AnimatorStates[0].TransitionInfo), mState.AnimatorStates[0].StateInfo.normalizedTime), 4);
                //Log.FileScreenWrite("isAnyState:" + mState.AnimatorStates[0].TransitionInfo.anyState, 5);

                //Log.FileWrite("");

                //Log.FileWrite("AC.Update() DT:" + Time.deltaTime.ToString("0.000000") + " IsG:" + mState.IsGrounded + "|" + mCharController.isGrounded + " GD:" + mState.GroundDistance.ToString("0.000") + " IMag:" + mState.InputMagnitudeTrend.Value.ToString("0.000") + " IMagAvg:" + mState.InputMagnitudeTrend.Average.ToString("0.000") +  " CamMode:" + _CameraRig.Mode + " CAngle:" + mState.InputFromCameraAngle.ToString("0.000") + " AAngle:" + mState.InputFromAvatarAngle.ToString("0.000") + " Motion:" + CurrentMotionName + " Phase:" + GetAnimatorMotionPhase(0) + " Pos:" + StringHelper.ToString(transform.position) + " Vel:" + StringHelper.ToString(mState.Velocity) + " AccVel:" + StringHelper.ToString(mAccumulatedVelocity) + " Rot:" + StringHelper.ToString(transform.rotation) + " Anim:" + AnimatorHashToString(mState.AnimatorStates[0].StateInfo, mState.AnimatorStates[0].TransitionInfo) + " AT:" + mState.AnimatorStates[0].StateInfo.normalizedTime.ToString("0.000") + " TT:" + mState.AnimatorStates[0].TransitionInfo.normalizedTime.ToString("0.000") + " RM-Vel:" + StringHelper.ToString(mRootMotionVelocity));

#if MC_ENABLE_PROFILING
                if (InputManagerStub.IsJustPressed(KeyCode.P))
                {
        ...
        public float GroundCast(ref RaycastHit rCollisionInfo)
        {
            ...
            Vector3 lRayStart = transform.position + (transform.rotation * mActorCenter);
            float lRayDistance = mActorToGroundDistance;

            // Test the collision and set the ground distance
            if (SafeRaycast(lRayStart, Vector3.down, ref rCollisionInfo, 10f))
            {
                lDistance = rCollisionInfo.distance - lRayDistance;
            }
        ...
        {
            mCharController.height = mBaseColliderHeight;
            mCharController.center = mBaseColliderCenter;
        }
        ...
        private void ProcessInput()
        {
            ...
            if (lVInput == 0f && lHInput == 0f)
            {
                ...
                ootiiInputStub.InputFromCameraAngle = float.NaN;
                ootiiInputStub.InputFromAvatarAngle = float.NaN;
            ...
            ootiiInputStub.InputFromCameraAngle = (mState.InputMagnitudeTrend.Value == 0f ? float.NaN : mState.InputFromCameraAngle);
            ootiiInputStub.InputFromAvatarAngle = (mState.InputMagnitudeTrend.Value == 0f ? float.NaN : mState.InputFromAvatarAngle);
        }
        ...
        {
            Vector3 lRayStart = transform.position + (transform.rotation * mActorCenter);
            Vector3 lRayDirection = Vector3.down;
            float lRayDistance = mActorToGroundDistance;

            if (SafeRaycast(lRayStart, lRayDirection, ref sGroundCollisionInfo, lRayDistance + 1f))            
            {
                mState.GroundNormal = sGroundCollisionInfo.normal;
                mState.GroundDistance = sGroundCollisionInfo.distance - lRayDistance;
            ...
            if (!mState.IsGrounded && mState.GroundDistance < mMaxStepHeight)
            {
                if (mState.GroundAngle > (mMaxSlopeAngle / 2f))
                {
                    mState.IsGrounded = true;
                }
            }
            ...
            if (!mState.IsGrounded && IsGroundedExpected)
            {
                ...
                lRayStart = transform.position + (transform.rotation * mActorCenter);
                lRayDirection = Vector3.down;
                lRayDistance = mActorToGroundDistance;

                Ray lRay = new Ray(lRayStart, lRayDirection);
                ...
                lSphereHits = UnityEngine.Physics.SphereCastAll(lRay, mActorRadius * 1.5f, lRayDistance + 0.01f, PlayerMask);
        ...
        private void ApplyMovement(float rDeltaTime)
        {
            ...
            if (lIsGravityEnabled)
            {
                ...
                // If we're grounded without a slope, cancel the gravity
                else if (mState.IsGrounded)
                {
                    lGravityAcceleration = Vector3.zero;
                }
            ...
            if (Mathf.Abs(lGroundDistance) < 0.01f) { lGroundDistance = 0f; }
            ...
            if (mState.IsGrounded && lIsGravityEnabled)
            {
                if (mState.GroundAngle > 0f)
                {
                    float lSlopeSinkAngle = Mathf.Min(Mathf.Abs(mState.GroundAngle), 45f);
                    mActorYPositionTarget = -(lGroundDistance + (_MaxSlopeSink * (lSlopeSinkAngle / 45f)));

                    mActorYPositionAdjust = mActorYPositionTarget;
                }
                else if (lGroundDistance == 0f)
                {
                    mActorYPositionAdjust = 0f;
                    mActorYPositionTarget = 0f;
                }
                else if (lGroundDistance < -0.01f)
                {
                    mActorYPositionAdjust = 0f;
                    mActorYPositionTarget = -lGroundDistance;
                }
                else if (lGroundDistance > 0.01f)
                {
                    mActorYPositionAdjust = 0f;
            ...
            // If we're moving twoards a vertical position, lerp there
            float lActorYPositionLerp = (mActorYPositionTarget < 0f ? _ActorStepDownLerp : _ActorStepUpLerp);
            mActorYPositionAdjust = Mathf.Lerp(mActorYPositionAdjust, mActorYPositionTarget, lActorYPositionLerp);
            if (Mathf.Abs(mActorYPositionTarget - mActorYPositionAdjust) < 0.01f) { mActorYPositionAdjust = mActorYPositionTarget; }

            // If we have an adjust, apply it
            if (mActorYPositionAdjust != 0f)
            {
                Vector3 lGroundDistanceAdjust = new Vector3(0, mActorYPositionAdjust, 0);
...
#else
                if (rState.AnimatorStates[i].AutoClearMotionPhase && rState.AnimatorStates[i].TransitionInfo.fullPathHash == 0)
#endif
                {
        ...
        private void FreeColliderFromEdge()
        {
            ...
            if (!mState.IsGrounded && mCharController.isGrounded)
            {
                ...
                bool lSphereHit = UnityEngine.Physics.SphereCast(lRayStart, mActorRadius, Vector3.down, out lSphereCollisionInfo, 10f, PlayerMask);
                if (lSphereHit)
                {
                    // We only want to handle the case when we're falling off 
                    // an edge. We don't want to do this with stairs or ramps
                    if (mState.GroundDistance > mMaxStepHeight)
                    {
        ...
        public string CurrentAnimatorStateName
        {
            get
            {
                return AnimatorHashToString(mState.AnimatorStates[0].StateInfo, mState.AnimatorStates[0].TransitionInfo);
            }
        }
