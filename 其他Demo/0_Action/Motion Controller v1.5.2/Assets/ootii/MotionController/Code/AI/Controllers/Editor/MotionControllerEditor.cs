...
public class MotionControllerEditor : Editor
{
    ...
    void OnEnable()
    {
        ...
        // Load the textures
        mBackground = Resources.Load<Texture>("mc_background");
        mItemSelector = Resources.Load<Texture>("mc_dot");
        mItemBorder = Resources.Load<Texture>("Border");
    ...
    public override void OnInspectorGUI()
    {
        ...
        SerializedProperty lMaxSlopeSinkSP = mMotionControllerSO.FindProperty("_MaxSlopeSink");
        float lMaxSlopeSink = EditorGUILayout.FloatField(new GUIContent("Max Slide Sink", "Max amount to sink the avatar while on a slope."), lMaxSlopeSinkSP.floatValue);
        ...
        SerializedProperty lStepUpLerpSP = mMotionControllerSO.FindProperty("_ActorStepUpLerp");
        float lStepUpLerp = EditorGUILayout.FloatField(new GUIContent("Step Up Lerp", "Speed at which the character moves up a step."), lStepUpLerpSP.floatValue);

        SerializedProperty lStepDownLerpSP = mMotionControllerSO.FindProperty("_ActorStepDownLerp");
        float lStepDownLerp = EditorGUILayout.FloatField(new GUIContent("Step Down Lerp", "Speed at which the character moves down a step."), lStepDownLerpSP.floatValue);
        ...
        if (GUI.changed)
        {
            ...
            lIsDirty = (lIsDirty || (lMaxSlopeSinkSP.floatValue != lMaxSlopeSink));
            lIsDirty = (lIsDirty || (lMaxSpeedSP.floatValue != lMaxSpeed));
            lIsDirty = (lIsDirty || (lRotationSpeedSP.floatValue != lRotationSpeed));
            lIsDirty = (lIsDirty || (lStepUpLerpSP.floatValue != lStepUpLerp));
            lIsDirty = (lIsDirty || (lStepDownLerpSP.floatValue != lStepDownLerp));
            ...
            if (lIsDirty)
            {
                ...
                lMaxSlopeSinkSP.floatValue = lMaxSlopeSink;
                lMaxSpeedSP.floatValue = lMaxSpeed;
                lRotationSpeedSP.floatValue = lRotationSpeed;
                lStepUpLerpSP.floatValue = lStepUpLerp;
                lStepDownLerpSP.floatValue = lStepDownLerp;
            }
        }
