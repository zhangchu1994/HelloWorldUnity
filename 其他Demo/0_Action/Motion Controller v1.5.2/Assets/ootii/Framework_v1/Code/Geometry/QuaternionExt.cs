...
namespace com.ootii.Geometry
{
    ...
    public static class QuaternionExt
    {
        ...
        {
            if (Mathf.Abs(rLeft.x - rRight.x) > QuaternionExt.Epsilon) { return false; }
            if (Mathf.Abs(rLeft.y - rRight.y) > QuaternionExt.Epsilon) { return false; }
            if (Mathf.Abs(rLeft.z - rRight.z) > QuaternionExt.Epsilon) { return false; }
            if (Mathf.Abs(rLeft.w - rRight.w) > QuaternionExt.Epsilon) { return false; }
        ...
        {
            if (rThis == Quaternion.identity) { return true; }
            if (rThis == QuaternionExt.InverseIdentity) { return true; }
        ...
        {
            float cos_theta = Vector3.Dot(u.normalized, v.normalized);
            float angle = Mathf.Acos(cos_theta);
            Vector3 w = (Vector3.Cross(u, v)).normalized;
            return Quaternion.AngleAxis(angle, w);
        }
