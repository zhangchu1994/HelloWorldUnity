...
set ERLANG_APP_NAME=uzone_node
for /F "usebackq tokens=1,2 delims==" %%i in (`wmic os get LocalDateTime /VALUE 2^>NUL`) do if '.%%i.'=='.LocalDateTime.' set ldt=%%j
set ldt=%ldt:~0,8%_%ldt:~8,6%

start "" %CompletePath% -pa ebin\ -boot start_sasl -sasl sasl_error_logger "{file, \"logs\\node_%ldt%.log\"}" -sname uzone_node -setcookie uzone_cookie -eval "application:start(uz_node)" -K true
