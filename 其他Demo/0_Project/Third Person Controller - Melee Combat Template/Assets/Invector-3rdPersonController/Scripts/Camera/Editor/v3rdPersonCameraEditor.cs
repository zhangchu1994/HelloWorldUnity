...
public class v3rdPersonCameraEditor : Editor 
{
	...
	void StateData(v3rdPersonCameraState camState)
	{
        EditorGUILayout.Space();
    ...
    void DrawLookPoint(v3rdPersonCameraState camState)
    {
    ...
    void FreeDirectionalMode(v3rdPersonCameraState camState)
    {
    ...
    void FixedAngleMode(v3rdPersonCameraState camState)
    {
        camState.defaultDistance = EditorGUILayout.FloatField("Distance", camState.defaultDistance);
        camState.useZoom = EditorGUILayout.Toggle("Use Zoom", camState.useZoom);
        if (camState.useZoom)
    ...
    void FixedPointMode(v3rdPersonCameraState camState)
    {          
        //camState.height = EditorGUILayout.FloatField("Height", camState.height);
        camState.smoothFollow = EditorGUILayout.FloatField("Smooth Follow", camState.smoothFollow);       
        //camState.right = EditorGUILayout.Slider("Right", camState.right, -3f, 3f);
	...
	{
		foreach (v3rdPersonCameraState state in tpCamera.CameraStateList.tpCameraStates) 		
	...
	{
		vScriptableObjectUtility.CreateAsset<v3rdPersonCameraListData>();
	}

	private string[] getListName(List<v3rdPersonCameraState> list)
	{
