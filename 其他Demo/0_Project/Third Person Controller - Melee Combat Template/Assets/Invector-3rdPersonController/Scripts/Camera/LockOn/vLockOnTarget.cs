...
namespace Invector
{
    ...
    public abstract class vLockOnTarget : MonoBehaviour
    {
        ...
        {
            if (v3rdPersonCamera.instance != null && v3rdPersonCamera.instance.target != null)
                watcher = v3rdPersonCamera.instance.target;
            else
                watcher = transform;
            var listPrimary = new List<Transform>();
            var targets = Physics.SphereCastAll(watcher.position, range, watcher.forward, .01f);
			foreach(RaycastHit hitOther in targets)          
            {
        ...
        protected void SortTargets(ref List<Transform> list)
        {
			...
			foreach(Transform _transform in list)           
            {
