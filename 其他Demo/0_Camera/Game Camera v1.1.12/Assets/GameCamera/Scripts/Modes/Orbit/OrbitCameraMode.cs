...
namespace RG_GameCamera.Modes
{
    ...
    public class OrbitCameraMode : CameraMode
    {
        ...
        private void PanCameraDelta(Vector3 delta)
        {
            ...
            {
                var limitX = config.GetVector2("DragLimitX");
                var limitY = config.GetVector2("DragLimitY");
                var limitZ = config.GetVector2("DragLimitZ");

                var cpos = UnityCamera.transform.position;

                cpos.x = Mathf.Clamp(cpos.x + delta.x, limitX.x, limitX.y);
                cpos.y = Mathf.Clamp(cpos.y + delta.y, limitY.x, limitY.y);
                cpos.z = Mathf.Clamp(cpos.z + delta.z, limitZ.x, limitZ.y);

                cameraTarget += UnityCamera.transform.position - cpos;
                UnityCamera.transform.position = cpos;
            }
            else
            {
