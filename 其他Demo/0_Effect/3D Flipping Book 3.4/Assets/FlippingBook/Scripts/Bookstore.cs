  ...
  RaycastHit myhit = new RaycastHit();
	...
	void LateUpdate(){
		if(cammove==true){
			...
			GameObject.Find ("Main Camera").transform.position=Vector3.MoveTowards(new Vector3(GameObject.Find ("Main Camera").transform.position.x,2.865031f,-4.295908f),new Vector3(GameObject.Find(""+booknum+"").transform.position.x-1,2.865031f,-4.295908f),speedcammovement);
				
			if(GameObject.Find ("Main Camera").transform.position.x==GameObject.Find(""+booknum+"").transform.position.x){
    ...
    if (Input.GetMouseButton(0))
    {
        if (Physics.Raycast(myray, out myhit,1000000)){
				...
				if(myhit.transform.name == "Box03"){
                    ...
                    Application.LoadLevel("ExampleBook");
					}
					else
					{
						 Application.LoadLevel(BookstoreBookApps[bookNumber]);
					}
    ...
    IEnumerator LoadBooksFromXML()
    {
      downloadiconbool=true;
        ...
        if (BookstoreEnabled)
        {
            if (Localxml)
            {
                URLString4 = "file://" + Application.dataPath + "/FlippingBook/Resources/books.xml";
            }
            else
            {
             //   URLString4 = "" + XMLBOOKSURL + "";
				URLString4 = "" + XMLBOOKSURL + ""+"books.xml";
            }
            Debug.Log(URLString4);
            WWW ww4 = new WWW(URLString4);
            yield return ww4;
            string XmlString4 = ww4.data;
			...
			WWW w = null;
				if (Localxml)
            {
            w = new WWW("file://" + Application.dataPath+"/FlippingBook/Resources/"+ thisnode.Attributes["coverimage"].Value);
             }
		    else
			{
         //   w = new WWW(thisnode.Attributes["coverimage"].Value);
			...
			clone.transform.Find("book/Box03").GetComponent<Renderer>().material.mainTexture=tex;
				...
				if (EnableBooksSize){
				//	clone.transform.localScale = new Vector3(float.Parse(thisnode.Attributes["x"].Value),1f,float.Parse(thisnode.Attributes["y"].Value)); 
				...
				//	clone.transform.Find("book/Plane").transform.localScale= new Vector3(float.Parse(thisnode.Attributes["x"].Value),1f,float.Parse(thisnode.Attributes["y"].Value)); 
