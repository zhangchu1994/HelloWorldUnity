Shader "Own/UISpriteShine"
{
	Properties
	{
		...
		_Speed ("Shine Speed", Range(0.0,10.0) ) = 0.5
		_EffectRange ("Cutout threshold", Range(0.0,0.5) ) = 0.2
	...
	SubShader
	{
		...
		Pass
		{
			...
			struct v2f
			{
				...
				half2 reflectionParam : TEXCOORD1;
				fixed4 color : COLOR;
			};
			...
			v2f vert (appdata_t v)
			{
				...
				o.color = v.color;
				float s = _Time.y * _Speed;
				o.reflectionParam.x = (s - (int)s) * 2.0f;//(tan(_Time.a * _Speed / 5.0f));
				o.reflectionParam.y = o.reflectionParam.x;//(tan(_Time.a * _Speed / 5.0f));
				return o;
			}
