	...
	SubShader {
		...
		Pass {
			...
			v2f vert (appdata v)
			{
				...
				o.cubenormal = mul (UNITY_MATRIX_MV, float4(Normal_normalize,0.0f)).xyz;
			...
			{
				float4 col = _Color * tex2D(_MainTex, i.texcoord.xy);
				float4 localCube = texCUBE(_ToonShade, i.cubenormal);
				
				return float4(2.0f * localCube.rgb * col.rgb, col.a)+(1.0f-i.texcoord.z) * float4(1.0f,1.0f,1.0f,1.0f) * _AttackSet;
			}
			ENDCG			
		}
	} 
