 ...
 IDictionary<string, object>
    {
		...
		public JsonObject Clone () {
			...
			foreach (var kv in _members) {
				if (kv.Value is JsonObject) {
					jo[kv.Key] = ((JsonObject)kv.Value).Clone();
				} else {
					jo[kv.Key] = kv.Value;
				}
			}
		...
		/// Gets the keys array.
		/// </summary>
		/// <value>The keys array.</value>
		public string[] KeysArray
		{
			get { 
					string[] ks = new string[_members.Count];
					_members.Keys.CopyTo(ks,0);
					return ks; 
				}
		}
		...
		public object TryGet(params object[] keys){
			JsonObject m = this;
			object mo = null;
			for (int i = 0; i < keys.Length - 1; i++) {
				if(m.ContainsKey(keys[i].ToString())){
					mo = m[keys[i].ToString()];
					//Debug.Log(mo.GetType());
					if(mo!=null && mo.GetType() == typeof(JsonObject)){
						m = (JsonObject)mo;
					}else{
						return null;
					}
			...
			if (m.ContainsKey (keys [keys.Length - 1].ToString())) {
				return m [keys [keys.Length - 1].ToString()];
			} else {
				return null;
			}
		}
		...
		public bool TrySet(params object[] keysAndValue){
			JsonObject m = this;
			object mo = null;
			for (int i = 0; i < keysAndValue.Length - 2; i++) {
				if(m.ContainsKey(keysAndValue[i].ToString())){
					mo = m[keysAndValue[i].ToString()];
					//Debug.Log(mo.GetType());
					if(mo.GetType() == typeof(JsonObject)){
						m = (JsonObject)mo;
					}else{
						Debug.LogError("*******Has Key:" + keysAndValue[i].ToString() + ",But the Value is not Json!!*******");
						return false;
					}
				}else{
					JsonObject tmp = new JsonObject();
					m[keysAndValue[i].ToString()] = tmp;
					m = (JsonObject)m[keysAndValue[i].ToString()];
				}
			}
			m [keysAndValue [keysAndValue.Length - 2].ToString()] = keysAndValue [keysAndValue.Length - 1];
			return true;
		}
