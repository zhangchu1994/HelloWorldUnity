...
namespace GlobalGame 
{
	...
	//	int LogCount = 0;
		...
		int m_xOffSite = 18;//30
		int m_yOffSite = -12;//13
		GameObject m_rootObj;

		void Awake()
		{
			m_rootObj = GameObject.Find ("GameObject");
		}
	...
	//		int x = 18;
	//		int y = 18;
	//		cam.transform.position = new Vector3 (x*10+m_xOffSite,30,y*10+m_yOffSite);
	...
	//		70显示8列
			...
			int width = 10;
			int height = 10;
			int offetX = (int)(cam.transform.position.x - m_xOffSite)/width;
			int offetZ = (int)(cam.transform.position.z - m_yOffSite)/height;
			int offetX1 = (int)(cam.transform.position.x - m_xOffSite);
			int offetZ1 = (int)(cam.transform.position.z - m_yOffSite);
			...
			for (int i = -1; i < 6; i++) 
			{
				...
				for (int j = -1; j < 8; j++) 
				{
					...
					if (IsExist (name) == false) 
					{
						...
						ActorObject.transform.SetParent (m_rootObj.transform);
