...
function SkillMainPanel.OnCreate(obj)
    ...
    m_SystemMainPanel.InitView(obj,this,3);--1角色 2转生 3羽翼 4经脉
end

function SkillMainPanel.OnDownButtonClick(go,argIndex)
    if (argIndex == 1) then
        panelMgr:CreatePanel('UI/Skill/SkillPanel', 'UICamera/Canvas/SkillMainPanel','SkillPanel', m_SystemMainPanel.OnSubViewCreate);
    elseif (argIndex == 2) then
        panelMgr:CreatePanel('UI/Skill/SkillSecretPanel', 'UICamera/Canvas/SkillMainPanel','SkillSecretPanel', m_SystemMainPanel.OnSubViewCreate);
    elseif (argIndex == 3) then
        panelMgr:CreatePanel('UI/Skill/SkillBreakPanel', 'UICamera/Canvas/SkillMainPanel','SkillBreakPanel', m_SystemMainPanel.OnSubViewCreate);
    end
end
